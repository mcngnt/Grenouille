...
public class MyBot : IChessBot
{
    ...
    public float Search(Board board, float alpha, float beta, int depth, int startingDepth, Timer timer, bool isQuiescence, int hasCastled)
    {
        ...
        else
        {
            ...
            float currentEval = (board.IsInCheck() ? -5 : 0) - (Math.Abs(board.GetKingSquare(true).File - board.GetKingSquare(false).File) + Math.Abs(board.GetKingSquare(true).Rank - board.GetKingSquare(false).Rank)) * endGameCoef * 2;
